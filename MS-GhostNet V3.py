...
class MCF2D(nn.Module):
    def __init__(self,c2=12,ks=(3,5,7)):
        super(MCF2D,self).__init__()
        self.m = nn.ModuleList(Conv2D(c2,k) for k in ks)
        #self.conv = nn.Conv2d(48,3,1,1) # +

    def forward(self,x):
        #通过每个卷积层处理输入，并将结果储存到fea中
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, dw_kernel_size=3,
        ...
        has_scsa = se_ratio is not None and se_ratio > 0.
        self.stride = stride
        ...
        if has_scsa:
            self.scsa = SCSA(dim=mid_chs,head_num=4, window_size=7)
        else:
            self.scsa = None
...
class GhostNet(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.2, block=GhostBottleneck, args=None):
        ...
        self.conv_stem = MCF2D(c2=16,ks=(3,5,7))
        output_channel = 16*3
...
def ghostnetv3(**kwargs):
    ...
    return GhostNet(cfgs, num_classes=18, width=kwargs['width'], dropout=0.2)


if __name__ == '__main__':
    net = ghostnetv3(width=0.5)
